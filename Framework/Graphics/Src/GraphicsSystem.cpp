...
LRESULT CALLBACK GraphicsSystem::GraphicSystemMessageHandler(HWND handle, UINT message, WPARAM wParam, LPARAM lParam)
{
...
{
    ASSERT(sGraphicsSystem == nullptr, "GraphicsSystem: is already initializes");
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
    ...
    if (width != GetBackBufferWidth() || height != GetBackBufferHeight())
    {
        ...
        ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to access swap cahin");
    ...
    ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed tp create render target");

    // creating the depth stencil buff
